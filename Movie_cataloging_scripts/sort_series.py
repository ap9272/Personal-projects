...
def shortcuts(list_movies,files):
  ...
  for i in range(No_of_Movies):
    ...
    try:
      ...
      d = dest_dir + '\\Troublesome'
      if not os.path.exists(d):
          os.makedirs(d)
      createshortcut(d, movie, source_dir, full_name, '')

list_series = os.listdir(series_dir)

No_of_series = len(list_series)

for s in range(No_of_series):
    source_dir = str(series_dir + "\\" + list_series[s])
    print str(s+1) + " of " + str(No_of_series) + " series done."
